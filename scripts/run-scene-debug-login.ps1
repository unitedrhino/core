...
$CodeIdFile = Join-Path $env:TEMP "lianxi-captcha-codeid.txt"
...
if ($CodeId -eq "" -and (Test-Path $CodeIdFile)) {
    $CodeId = (Get-Content $CodeIdFile -Raw).Trim()
    Write-Host "Using saved codeID=$CodeId"
}

if ($CodeId -eq "") {
    Write-Host "No codeID - fetching captcha..."
    ...
    Set-Content -Path $CodeIdFile -Value $CodeId -NoNewline
}

Write-Host "Login with codeID=$CodeId captcha=$CaptchaCode"
$loginObj = @{
    loginType  = "pwd"
    tenantCode = "default"
    account    = $Account
...
if ($login.code -ne 200) {
    ...
    Remove-Item $CodeIdFile -ErrorAction SilentlyContinue
    exit 1
}
...
if ($tplCount -eq 0) {
    ...
    if ($created.code -eq 200) {
        ...
        Write-Host "create fail $($created.msg)" -ForegroundColor Red
    }
} else {
...
Write-Host ""
Write-Host "=== MESSAGES (group) BEFORE ===" -ForegroundColor Cyan
...
if ($null -eq $targetScene) {
    Write-Host "No scene with notify/ruleScene found" -ForegroundColor Yellow
} else {
    ...
    if ($totalAfter -gt $totalBefore) {
        ...
        Write-Host "No new message" -ForegroundColor Yellow
    }
}
