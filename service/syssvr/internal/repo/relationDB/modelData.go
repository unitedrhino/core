...
type SysDataOpenAccess struct {
	...
	UserID       int64               `gorm:"column:user_id;uniqueIndex:idx_tc_ac;type:bigint;NOT NULL"`
	Code         string              `gorm:"column:code;type:VARCHAR(50);uniqueIndex:idx_tc_ac;NOT NULL"` //用来标识用来干嘛的
...
type SysDataArea struct {
	...
	TargetType     def.TargetType      `gorm:"column:target_type;uniqueIndex:idx_ri_mi;type:VARCHAR(50);NOT NULL"`
	TargetID       int64               `gorm:"column:target_id;uniqueIndex:idx_ri_mi;type:bigint;NOT NULL"`          // 授权对象的id,角色id,用户id
	ProjectID      dataType.ProjectID  `gorm:"column:project_id;uniqueIndex:idx_ri_mi;type:bigint;NOT NULL"`         // 所属项目ID(雪花ID)
	AreaID         int64               `gorm:"column:area_id;uniqueIndex:idx_ri_mi;type:bigint;NOT NULL"`            // 区域ID(雪花ID)
	AreaIDPath     string              `gorm:"column:area_id_path;uniqueIndex:idx_ri_mi;type:VARCHAR(256);NOT NULL"` // 区域ID(雪花ID)
...
type SysUserAreaApply struct {
	ID         int64               `gorm:"column:id;type:bigint;primary_key;AUTO_INCREMENT"`
	TenantCode dataType.TenantCode `gorm:"column:tenant_code;uniqueIndex:idx_ri_mi;type:VARCHAR(50);NOT NULL"` // 租户编码
	UserID     int64               `gorm:"column:user_id;uniqueIndex:idx_ri_mi;type:bigint;NOT NULL"`          // 用户ID(雪花id)
	ProjectID  dataType.ProjectID  `gorm:"column:project_id;uniqueIndex:idx_ri_mi;type:bigint;NOT NULL"`       // 所属项目ID(雪花ID)
	AreaID     dataType.AreaID     `gorm:"column:area_id;uniqueIndex:idx_ri_mi;type:bigint;NOT NULL"`          // 区域ID(雪花ID)
...
type SysDataProject struct {
	ID         int64               `gorm:"column:id;type:bigint;primary_key;AUTO_INCREMENT"`
	TenantCode dataType.TenantCode `gorm:"column:tenant_code;uniqueIndex:idx_ri_mi;type:VARCHAR(50);default:default"` // 租户编码
	ProjectID  int64               `gorm:"column:project_id;uniqueIndex:idx_ri_mi;type:bigint;NOT NULL"`              // 所属项目ID(雪花ID)
	TargetType def.TargetType      `gorm:"column:target_type;uniqueIndex:idx_ri_mi;type:VARCHAR(50);NOT NULL"`
	TargetID   int64               `gorm:"column:target_id;uniqueIndex:idx_ri_mi;type:bigint;NOT NULL"` // 授权对象的id,角色id,用户id
