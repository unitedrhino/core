...
func (p SlotInfoRepo) MultiInsert(ctx context.Context, data []*SysSlotInfo) error {
	err := p.db.WithContext(ctx).Clauses(clause.OnConflict{DoNothing: true, Columns: stores.SetColumnsWithPg(p.db, &SysSlotInfo{}, "idx_sys_slot_info_code_slot")}).Model(&SysSlotInfo{}).Create(data).Error
	return stores.ErrFmt(err)
}
