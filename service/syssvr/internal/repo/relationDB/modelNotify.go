...
type SysNotifyConfigTemplate struct {
	ID         int64                          `gorm:"column:id;type:BIGINT;primary_key;AUTO_INCREMENT"`                                              // id编号
	TenantCode dataType.TenantCodeWithCommonR `gorm:"column:tenant_code;type:VARCHAR(50);uniqueIndex:idx_sys_notify_config_template_ri_mi;NOT NULL"` // 租户编码
...
type SysNotifyChannel struct {
	ID         int64                          `gorm:"column:id;type:BIGINT;primary_key;AUTO_INCREMENT"`                               // id编号
	TenantCode dataType.TenantCodeWithCommonR `gorm:"column:tenant_code;type:VARCHAR(50);NOT NULL"`                                   // 租户编码,为common是公共的
