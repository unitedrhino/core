...
type SysAccessInfo struct {
	...
	Code       string          `gorm:"column:code;type:VARCHAR(100);uniqueIndex:idx_sys_access_info_app_route;NOT NULL"` // 请求名称
	...
	stores.NoDelTime
	DeletedTime stores.DeletedTime `gorm:"column:deleted_time;default:0;uniqueIndex:idx_sys_access_info_app_route"`
...
type SysApiInfo struct {
	...
	Method        string `gorm:"column:method;uniqueIndex:idx_sys_api_info_route;type:VARCHAR(50);NOT NULL"` // 请求方式（1 GET 2 POST 3 HEAD 4 OPTIONS 5 PUT 6 DELETE 7 TRACE 8 CONNECT 9 其它）
	Route         string `gorm:"column:route;uniqueIndex:idx_sys_api_info_route;type:VARCHAR(100);NOT NULL"` // 路由
	Name          string `gorm:"column:name;type:VARCHAR(100);NOT NULL"`                                     // 请求名称
...
type SysTenantAccess struct {
	...
	TenantCode dataType.TenantCode `gorm:"column:tenant_code;uniqueIndex:idx_sys_tenant_access_tenant_scope;type:VARCHAR(50);NOT NULL;"` // 租户编码
	AccessCode string              `gorm:"column:access_code;uniqueIndex:idx_sys_tenant_access_tenant_scope;type:VARCHAR(50);NOT NULL"`  // 范围编码
	stores.NoDelTime
	DeletedTime stores.DeletedTime `gorm:"column:deleted_time;default:0;uniqueIndex:idx_sys_tenant_access_tenant_scope"`
}
