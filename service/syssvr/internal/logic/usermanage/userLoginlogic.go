...
func gentLoginKey(appID, code string) string {
	return fmt.Sprintf("sys:user:wxak:login:%s:%s", appID, code)
}

func StoreWxLoginResAccessToken(ctx context.Context, appID, code string, tk *oauth.ResAccessToken) error {
	return caches.GetStore().SetexCtx(ctx, gentLoginKey(appID, code), utils.MarshalNoErr(tk), 10*60)
}

func DelWxLoginResAccessToken(ctx context.Context, appID, code string) error {
	_, err := caches.GetStore().DelCtx(ctx, gentLoginKey(appID, code))
	return err
}

func GetWxLoginResAccessToken(ctx context.Context, appID, code string) (*oauth.ResAccessToken, error) {
	ret, err := caches.GetStore().GetCtx(ctx, gentLoginKey(appID, code))
	if err != nil {
		return nil, err
	}
...
func StoreWxRegisterResAccessToken(ctx context.Context, appID, code string, tk *oauth.ResAccessToken) error {
	DelWxLoginResAccessToken(ctx, appID, code)
